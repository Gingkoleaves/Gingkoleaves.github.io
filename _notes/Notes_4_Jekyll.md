...
* **`_includes/`**：存放可重用的代码片段（如 `footer.html`, `navigation.html`）。使用 `{% include filename.html %}` 引入。
...
* **对象 (Objects)**：用 `{{ }}` 包裹，输出内容。
    * `{{ page.title }}`：当前页面的标题。
    * `{{ site.time | date: "%Y-%B-%d" }}`：格式化输出当前时间。
* **标记 (Tags)**：用 `{% %}` 包裹，用于逻辑控制。
    * **条件判断**：`{% if page.tags contains 'jekyll' %} ... {% endif %}`
    * **循环**：`{% for post in site.posts limit:5 %} ... {% endfor %}`
* **过滤器 (Filters)**：通过管道符 `|` 修改输出。
    * `{{ "hello world" | upcase }}` $\rightarrow$ **HELLO WORLD**
    * `{{ content | strip_html | truncatewords: 20 }}`：生成文章摘要。
