...
在_ 通配符之外，提供基于变量接受值的方式，允许访问这个输入值【用通配符_ 承接则无法访问输入值】
...
可以let <struct>{<attr1>:<var1>}从结构体中提取属性的值到变量；可以部分解构，match p {  Point { x, y: 0 } => println!("On the x axis at {}", x)};可以解构嵌套的结构体、复杂的嵌套元组；可以解构定长不定长数组[用..]；可以用_ 忽略函数传入参数,_ 不导致转移权转移
...
> 3. 1. Work[j] = Work[j] + Allocation[i, j];
> 3. 2. Finish[i] = true;
> 跳转回步骤2
> 
